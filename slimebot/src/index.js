...
var slimeWordMap = new Map();
var blacklistWordMap = new Map();
var d = new Date();
var n = d.getTime();
class submitter {
    constructor(submitterName, time) {
...
client.once('ready', () => {
    if (fs.existsSync('wordList.json')) {
        let raw = fs.readFileSync('wordList.json');
        slimeWordMap = new Map(JSON.parse(raw));
    }
    if (fs.existsSync('blackList.json')) {
        let raw = fs.readFileSync('blackList.json');
...
client.on('message', async (message) => {
    ...
    var chatMessage = message.content.split(/ +/);
    //adding new words
    if (message.channel.type == "dm") {
        var args = chatMessage.slice(1, chatMessage.length);
        const command = chatMessage[0];
    ...
    if (message.channel.type != "dm") {
        if (chatMessage[0].toLowerCase() == "slimebot") {
            var args = chatMessage.slice(2, chatMessage.length);
            const command = chatMessage[1];
            ...
            if (matches) {
                ...
                (async function () {
                    ...
                    matches.forEach(element => {
                        ...
                        var wordmaker = client.users.get((_a = slimeWordMap.get(element)) === null || _a === void 0 ? void 0 : _a.submitterName);
